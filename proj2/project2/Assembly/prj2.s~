 ...
 .fill 0x00000000
 .fill 0x00000000 !15
main:	addi $sp, $zero, initsp ! initialize the stack pointer
	lw $sp, 0($sp)
	la $s0, ti_inthandler 	! Install timer interrupt handler into the vector table
	sw $s0, 0($zero)
!	ei 			!Don't forget to enable interrupts...
	...
	jalr $at, $ra		!run pow
	halt
POW:addi $sp, $sp, 2   ! push 2 slots onto the stack
...
DONE: jalr $ra, $zero		
ti_inthandler: add $k0, $ra, $zero
	ei
	addi $s0, $zero, 524286
	add $s0, $s0, $s0		!set $s0 to 0xFFFFC
	addi $s1, $zero, 59
	lw $s2, 0($s0)
	beq $s1, $s2, minute
	addi $s2, $s2, 1
